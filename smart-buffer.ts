...
const readAux = (
  ...
  const result = callback.call(buffer.view, byteOffset, littleEndian) as number;
  return result;
};
...
const ensureCapacity = (buffer: SmartBuffer, size: number) => {
  ...
  if (newSize > buffer.length && buffer.buffer != null) {
    const newBuffer = new ArrayBuffer(newSize);

    const view = new Uint8Array(newBuffer);
    view.set(new Uint8Array(buffer.toBuffer() as ArrayBuffer));
...
const stringWriters = {
  ...
  utf16: (buffer: SmartBuffer, value: string, byteOffset?: number | null) => {
    ...
    for (let i = 0; i < l; i++) {
      /* optimization */
      writers.u16.call(buffer.view, byteOffset + i * 2, value.charCodeAt(i));
    }
  },
};
...
export default class SmartBuffer {
   ...
   * @returns {string} The value read.
   */
  readEscapedStringNT(encoding: Encoding = "utf8", byteOffset?: number | null) {
    return decodeURIComponent(escape(this.readStringNT(encoding, byteOffset)));
  }
  ...
  writeEscapedString(
    ...
    this.writeString(unescape(encodeURIComponent(value)), encoding, byteOffset);
  }
